...
pub fn decode_hex(hex_str: &str) -> Result<Vec<u8>, String> {
    ...
    match decode(hex_str) {
        Ok(bytes) => Ok(bytes),
        Err(_) => Err("An error has occurred. Hex decode fail".to_string()),
    }
}
...
pub fn classify_script(script: &[u8]) -> ScriptType {
    ...
    if script == [0x76, 0xa9, 0x14] {
        ScriptType::P2PKH
    } else if script == [0x00, 0x14, 0xff] {
        ScriptType::P2WPKH
    } else {
        ScriptType::Unknown
    }
...
pub fn apply_fee(balance: &mut u64, fee: u64) {
    ...
    if *balance >= fee {
        ...
        *balance = 0; // Optionally handle negative logic
    }
}
